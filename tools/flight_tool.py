...
def guess_destination_city(query: str):
    """
    Cheap, non-LLM destination guess for the weather agent.

    Reuses the same route-parsing heuristics as the flight tool so we
    don't need a separate Groq call just to extract a city name.
    Returns a city name string, or None if nothing could be resolved.
    """

    _, arr_iata = parse_route(query)

    if not arr_iata:
        return None

    airport = AIRPORTS.get(arr_iata)

    if not airport:
        return None

    return airport.get("city") or airport.get("name")
